...
__version__ = "N/A"
build_date  = "Fri Apr 29 16:25:08 2011"
...
class Media(_Ctype):
    ...
    def get_tracks_info(self):
        ...
        @begincode
        L{MediaPlayer} *player = L{player_new_from_media}(media);
        L{add_option_flag}(media, "sout=\"#description\"");
        L{player_play}(player);
        // ... wait until playing
        L{player_release}(player);
        @endcode
...
class MediaPlayer(_Ctype):
    ...
    def video_get_aspect_ratio(self):
        ...
        @return: the video aspect ratio or NULL if unspecified (the result must be released with free()).
        '''
...
def libvlc_media_get_state(p_meta_desc):
    '''Get current state of media descriptor object. Possible media states
    are defined in libvlc_structures.c ( libvlc_NothingSpecial=0,
    libvlc_Opening, libvlc_Buffering, libvlc_Playing, libvlc_Paused,
    libvlc_Stopped, libvlc_Ended,
...
def libvlc_media_get_tracks_info(media):
    '''Get media descriptor's elementary streams description
    Note, you need to play the media _one_ time with --sout="#description"
    Not doing this will result in an empty array, and doing it more than once
    will duplicate the entries in the array each time. Something like this:
    @begincode
    L{MediaPlayer} *player = L{libvlc_media_player_new_from_media}(media);
    L{libvlc_media_add_option_flag}(media, "sout=\"#description\"");
    L{libvlc_media_player_play}(player);
    // ... wait until playing
    L{libvlc_media_player_release}(player);
    @endcode
    This is very likely to change in next release, and be done at the parsing
    phase.
    @param media: media descriptor object.
...
# 1 function(s) blacklisted:
#  libvlc_video_set_callbacks

# 9 function(s) not wrapped as methods:
